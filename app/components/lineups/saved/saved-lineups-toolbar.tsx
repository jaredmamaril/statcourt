...
export function SavedLineupsToolbar({
  ...
  const savedProfileMenuId = "saved-lineups-profile-menu";
  const savedSortMenuId = "saved-lineups-sort-menu";

  return (
    <>
      <div className="flex items-center justify-center gap-2 lg:gap-4">
        <label htmlFor="saved-lineups-search" className="sr-only">
          Search saved lineups
        </label>
        <input
          id="saved-lineups-search"
          type="search"
          value={savedLineupSearch}
          onChange={(event) => onSearchChange(event.target.value)}
          placeholder="Search saved lineups..."
          className="h-8 min-w-0 flex-1 rounded-md border border-[rgb(var(--court-accent-rgb)/0.22)] bg-[color:color-mix(in_srgb,var(--court-panel)_72%,transparent)] px-2 font-michroma text-[9px] text-white outline-none transition placeholder:text-white/55 focus:border-[rgb(var(--court-accent-rgb)/0.75)] focus:bg-[color:color-mix(in_srgb,var(--court-panel-alt)_78%,transparent)] lg:h-auto lg:max-w-md lg:px-4 lg:py-3 lg:text-xs"
        />
        ...
        <div className="relative shrink-0">
          <button
            ...
            aria-label="Filter saved lineups by stat profile"
            aria-expanded={openSavedDropdown === "profile"}
            aria-controls={savedProfileMenuId}
            aria-haspopup="true"
            className={`flex min-h-10 min-w-24 cursor-pointer items-center justify-between gap-1 rounded-md border px-2 font-michroma text-[8px] transition lg:min-h-0 lg:min-w-42 lg:gap-3 lg:px-4 lg:py-3 lg:text-xs ${
          ...
          {openSavedDropdown === "profile" && (
            <div id={savedProfileMenuId} className="absolute right-0 top-full z-100 mt-1 w-32 animate-[dropdownIn_140ms_ease-out_both] overflow-hidden rounded-md border border-white/15 bg-[var(--court-panel-alt)] shadow-[0_0_20px_rgba(0,0,0,0.45)] lg:mt-2 lg:w-full">
        ...
        <div className="relative shrink-0">
          <button
            ...
            aria-label="Sort saved lineups"
            aria-expanded={openSavedDropdown === "sort"}
            aria-controls={savedSortMenuId}
            aria-haspopup="true"
            className={`flex min-h-10 min-w-24 cursor-pointer items-center justify-between gap-1 rounded-md border px-2 font-michroma text-[8px] transition lg:min-h-0 lg:min-w-48 lg:gap-3 lg:px-4 lg:py-3 lg:text-xs ${
          ...
          {openSavedDropdown === "sort" && (
            <div id={savedSortMenuId} className="absolute right-0 top-full z-100 mt-1 w-32 animate-[dropdownIn_140ms_ease-out_both] overflow-hidden rounded-md border border-white/15 bg-[var(--court-panel-alt)] shadow-[0_0_20px_rgba(0,0,0,0.45)] lg:mt-2 lg:w-full">
      ...
      <div className="mt-2">
        <p className="mb-2 text-center font-michroma text-[8px] text-white/65 lg:mb-4 lg:text-xs">
          {savedLineupCount} Saved{" "}
          {savedLineupCount === 1 ? "Lineup" : "Lineups"}
        </p>

        <p className="text-center font-michroma text-[8px] uppercase text-white/60 lg:text-[9px]">
