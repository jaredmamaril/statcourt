...
export function SkillFilterDropdown({
  ...
  const dropdownId = "players-skill-filter-menu";
  ...
  return (
    <div className="relative">
      <button
        ...
        aria-label="Choose player rating filter"
        aria-expanded={isOpen}
        aria-controls={dropdownId}
        aria-haspopup="true"
        className="flex min-h-9 scale-[1.02] cursor-pointer items-center gap-1 rounded-md border border-[rgb(var(--court-accent-rgb)/0.95)] bg-[color:color-mix(in_srgb,var(--court-accent)_38%,var(--court-panel-alt))] px-2 font-michroma text-[9px] text-[var(--court-accent)] ring-1 ring-[rgb(var(--court-accent-rgb)/0.45)] transition-all duration-200 sm:min-h-8 sm:gap-2 sm:py-1 sm:text-xs"
      >
      ...
      {isOpen && (
        <div id={dropdownId} className="absolute left-0 top-full z-30 mt-1.5 max-h-44 w-22 overflow-y-auto rounded-md border border-white/20 bg-[var(--court-panel-alt)] py-1 shadow-xl animate-[dropdownIn_140ms_ease-out_both] sm:mt-2 sm:max-h-52 sm:w-40">
